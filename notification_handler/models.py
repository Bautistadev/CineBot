...
class Usuario(SQLModel, table=True):
    ...
    password: str
    Telefono: str
    generos: List["Genero"] = Relationship(back_populates="usuario", link_model="GeneroXUsuario")

class Genero(SQLModel, table=True):
    id: int = Field(default=None, primary_key=True)
    nombre: str
    peliculas: List["Pelicula"] = Relationship(back_populates="genero")
    usuarios: List["Usuario"] = Relationship(back_populates="genero", link_model="GeneroXUsuario")
